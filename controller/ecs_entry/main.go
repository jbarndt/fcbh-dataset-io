...
func main() {
	fmt.Println("****************** entering ecs_entry point ********************")
	...
	if err != nil {
		log.Error(ctx, 500, err, "Error Getting Object in Queue Input Folder. bucket: %s, key: %s", bucket, key)
		return
	}

	requestYaml, err2 := io.ReadAll(object.Body)
	_ = object.Body.Close()
	if err2 != nil {
		log.Error(ctx, 500, err, "Error reading yaml file from Queue Input Folder.")
	}

	fmt.Printf("Received request from bucket: %s, key: %s, request: %s\n", bucket, key, requestYaml)

	// control := NewController(ctx, requestYaml)
	// control.ProcessV2() // calls main entry point
	// var folder string
	// if status.IsErr {
	// 	folder := failedFolder
	// } else {
