...
func NewFairseqFA(ctx context.Context, conn db.DBAdapter, lang string, sttLang string) FairseqFA {
	var f FairseqFA
	f.ctx = ctx
	f.conn = conn
	f.lang = lang
...
func (f *FairseqFA) ProcessFiles(files []input.InputFile) dataset.Status {
...
func (f *FairseqFA) processFile(file input.InputFile, lang string) dataset.Status {
...
func (f *FairseqFA) forcedAlign(audioFile string, textFile string, lang string, tempDir string) (string, dataset.Status) {
	var result string
	var status dataset.Status
	MMSFAPYTHON := os.Getenv("FCBH_MMS_FA_PYTHON")
	pythonScript := filepath.Join(os.Getenv("GOPROJ"), "dataset/mms/fairseq_fa/align_and_segment.py")
...
func (f *FairseqFA) processPyOutput(file input.InputFile, outputFile string, references []int64) dataset.Status {
	var status dataset.Status
